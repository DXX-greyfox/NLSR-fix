...
ConfFileProcessor::processConfSectionGeneral(const ConfigSection& section)
{
  ...
  // load-aware-routing 配置解析
  try {
    std::string loadAwareStr = section.get<std::string>("load-aware-routing", "off");
    
    if (boost::iequals(loadAwareStr, "off") || boost::iequals(loadAwareStr, "false")) {
      m_confParam.setLoadAwareRouting(false);
    }
    else if (boost::iequals(loadAwareStr, "on") || boost::iequals(loadAwareStr, "true")) {
      m_confParam.setLoadAwareRouting(true);
    }
    else {
      std::cerr << "Invalid value for load-aware-routing: " << loadAwareStr << std::endl;
